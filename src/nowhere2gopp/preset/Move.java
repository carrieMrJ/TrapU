...
public class Move implements Serializable {
    ...
    public Move(final MoveType type) {
        if (type != MoveType.Surrender && type != MoveType.End)
            throw new IllegalArgumentException("constructors for surrender/end moves only");
	...
	if (string == null)
	    throw new SiteFormatException("cannot parse empty string!");
	...
	MoveType t;
	if (str.startsWith("("))
	    t = MoveType.AgentLink;
	else if (str.startsWith("{"))
	    t = MoveType.LinkLink;
	else
            throw new SitePairFormatException(msg);
