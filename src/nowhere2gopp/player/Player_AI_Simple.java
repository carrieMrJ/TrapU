 ...
 * Project 4f)
 * SimpleAI
 * !!! Unfortunately the SimpleAI only works for Size 3, Size 5 and Size 7 !!!!!!
 * for size 3 and 5 AI reacts very quickly, for size 7 it takes AI 20 second for a Move
 * and for size 9 and 11 it takes too much time
 * @author Xinying.He
 *
 */
public class Player_AI_Simple implements Player{
	...
	private int ourPathNum;
	private int enemyPathNum;
	
	
	
	/**
	 * help-Method for request(), to check if a Site is isolated (no link with other Sites)
	 * @param test the Site to check
	 * @param links current links
	 * @return if isolate return true else false
	 */
	public boolean isIsolate(Site test, Collection<SiteSet> links) {
	...
	public Move request() throws Exception, RemoteException {
		...
		/*in phase 1 we(the simpleAI) acts like randomAI*/
		...
		for(int i = 0; i < fakeMove1.size(); i++) {
			...
			for(int j = 0; j < fakeMove2.size(); j++) {
				 ...
				 */
				if(Checkerboard.isAgentBlocked(ourPos, enemyPosFake, links) || isIsolate(ourPos, links))
					causeLose = true;
				...
				if(Checkerboard.isAgentBlocked(ourPos, enemyPosFake, tempLinks) || isIsolate(ourPos, tempLinks)) 
					causeLose = true;

				ArrayList<Site> target = Checkerboard.getNode(tempLinks, enemyPosFake);
				CopyOnWriteArrayList<SiteSet> allSide = Checkerboard.getSide(tempLinks, enemyPosFake);
				/*the sum of available nodes (vertices) and sides (edges) from our perspective*/
				ourPathNum = target.size() + allSide.size();
				target = Checkerboard.getNode(tempLinks, ourPos);
				allSide = Checkerboard.getSide(tempLinks, ourPos);
				/*the sum of available nodes (vertices) and sides (edges) from enemy perspective*/
				enemyPathNum = target.size() + allSide.size();	
				
				/*if positive we have advantage, else we are disadvantaged*/
				difference = ourPathNum - enemyPathNum;
			...
			if(worstDiff > bestDiff && !causeLose) {
				bestDiff = worstDiff;
				idealMove = fakeMove1.get(i);
			}
		}//fakeMove1-loop ends
		...
		if(idealMove == null) {
			System.out.println("SimpleAI find that u will surely win the game in next turn");
